...
def load_and_clean_all_data(filenames):
    ...
    призначаючи кожному маршруту унікальний ID.
    """
    ...
    print(f"Успішно об'єднано. Загальна кількість рядків до очищення: {len(combined_data)}.")
...
def plot_final_results(full_data, attacked_data, true_anomaly_indices, detected_anomaly_indices, gpr_model):
    """
    Візуалізує результати, показуючи інтерпольовану карту на всіх трьох графіках.
    """
    ...
    ax1 = axes[0]
    grid_z_clean = gpr_model.predict(np.c_[grid_x.ravel(), grid_y.ravel()]).reshape(grid_x.shape)
    ...
    # --- 2. Інтерпольована карта (Після атаки) ---
    ax2 = axes[1]
    ...
    ax2.contourf(grid_x, grid_y, grid_z_attacked, cmap='viridis', levels=100, vmin=vmin, vmax=vmax)
    ...
    # --- 3. Результати детектора на чистій карті ---
    ax3 = axes[2]
    ax3.contourf(grid_x, grid_y, grid_z_clean, cmap='viridis', levels=100, vmin=vmin, vmax=vmax)
    ax3.contour(grid_x, grid_y, grid_z_clean, colors='white', alpha=0.4, levels=10)
    ax3.scatter(attacked_data.loc[detected_anomaly_indices, 'x'], attacked_data.loc[detected_anomaly_indices, 'y'],
                s=200, facecolors='none', edgecolors='red', linewidth=3, label='Виявлена аномалія')
    ax3.set_title('3. Результат роботи детектора', fontsize=14)
    ax3.legend()
    
    # Спільна кольорова шкала
